...
EndComputation();
  ...
  for (uint64_t i0 = 0; i0 < 1001; i0++) {
	prediction_vector[i0] = funcReconstruct2PCCons(Arr1DIdxRowM(tmp16, 1001, i0), 2) /
	  std::pow(4., kScale);
  }
  ClearMemSecret4(1, 1, 1, 1000, tmp16);
  if (party == CLIENT) {
	std::sort(prediction_vector.begin(), prediction_vector.end(),
			  [](double u, double v) { return u > v; });
	printf("top-10 values from Alexnet\n");
	printf("[");
...
for (uint64_t i0 =  (uint64_t)0; i0 <  (int32_t)1; i0++){
  for (uint64_t i1 =  (uint64_t)0; i1 <  (int32_t)1000; i1++){
    auto pred = funcReconstruct2PCCons(Arr3DIdxRowM(tmp169, 1, 1, 1, i0, i1, i2), 2);
    if (party == CLIENT) {
