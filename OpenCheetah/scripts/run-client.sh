...
then
  ...
  if ! contains "cheetah SCI_HE" $1; then
    echo -e "Usage: run-client.sh ${RED}[cheetah|SCI_HE]${NC} [sqnet|resnet50|densenet121|alexnet|shufflenet]"
	exit 1
  fi

  if ! contains "sqnet resnet50 densenet121 alexnet shufflenet" $2; then
    echo -e "Usage: run-client.sh [cheetah|SCI_HE] ${RED}[sqnet|resnet50|densenet121|alexnet|shufflenet]${NC}"
	exit 1
  fi
